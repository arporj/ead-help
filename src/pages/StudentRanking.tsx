...
import { Award, ShieldAlert, CheckCircle2, UserCheck } from 'lucide-react';
...
export const StudentRanking: React.FC = () => {
  ...
  // - Filter out students who haven't accepted the LGPD consent.
  // - Sort by points descending.
  const rankingList = students
    .filter(s => s.profile.lgpdRankingConsent || s.user.id === user?.id) // Keep current student even if no consent, but mark as private to others
    .map((s) => ({
      name: s.user.id === user?.id && !s.profile.lgpdRankingConsent ? `${s.user.name} (Você - Privado)` : s.user.name,
      points: s.profile.rankingPoints,
      isCurrentUser: s.user.id === user?.id,
      plan: s.profile.plan,
      isPrivate: s.user.id !== user?.id && !s.profile.lgpdRankingConsent
    }))
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-400 text-xs mt-1">
          Tabela de classificação baseada nos pontos acumulados em rodadas concluídas de 10 questões de simulado.
        </p>
      </div>
      ...
      {!consentGranted ? (
        ...
        <div className="bg-brand-medium/10 border border-brand-medium/40 p-6 rounded-2xl shadow-xl space-y-5">
          ...
          <div className="border border-brand-medium/55 bg-brand-dark/25 p-4 rounded-xl text-xs text-gray-400 space-y-2">
            ...
            <ul className="list-disc pl-4 space-y-1 text-[11px]">
              <li>Seu nome completo e total de pontos em simulados ficarão visíveis para todos os alunos logados no portal.</li>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-green-950/20 border border-green-500/25 p-3 rounded-xl flex items-center justify-between text-xs text-green-300">
            ...
            <button
              onClick={toggleLgpdConsent}
              className="text-[10px] hover:text-red-300 font-bold transition-colors"
            >
              Ocultar Meu Perfil
            </button>
          </div>
          ...
          <div className="bg-brand-medium/10 border border-brand-medium/40 rounded-2xl overflow-hidden shadow-xl">
            <div className="grid grid-cols-12 bg-brand-medium/20 p-4 border-b border-brand-medium/45 font-bold text-xs text-brand-light uppercase tracking-wider">
              ...
              <span className="col-span-6">Estudante</span>
              <span className="col-span-4 text-right">Pontuação</span>
            </div>
                  ...
                  >
                    ...
                    <div className="col-span-6 flex items-center gap-2">
                      ...
                      {item.isCurrentUser && !studentProfile?.lgpdRankingConsent && (
                        <span className="text-[8px] bg-red-950 text-red-300 border border-red-500/20 px-1.5 py-0.2 rounded font-bold uppercase ml-1.5 shrink-0">
                          Invisível aos outros
                        </span>
                      )}
                    </div>

                    <span className="col-span-4 text-right font-mono text-brand-light font-extrabold">
                      {item.points} pts
                    </span>
                  </div>
                );
